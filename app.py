...
# ==========================================
@st.cache_data # 🌟 캐싱 데코레이터: 똑같은 사진이 올라오면 연산을 생략하고 0.1초 만에 결과 반환!
def preprocess_image(image_bytes):
    # 1. 바이트 데이터를 PIL 이미지로 열기
    img = Image.open(io.BytesIO(image_bytes))
    
    # 2. 🌟 [핵심 수술] 사진 용량 강제 축소 (토큰 폭발 완벽 방지)
    # 가로세로 최대 800픽셀로 비율을 유지하며 압축합니다. 
    img.thumbnail((800, 800)) 
    
    # 3. OpenCV 흑백 변환 및 CLAHE (그림자 제거)
    ...
    # 4. 결과 반환 (UI에 띄울 압축된 원본과 AI에게 줄 전처리 이미지 모두 반환)
...
if uploaded_file is not None:
    # 🌟 업로드된 파일의 '순수 바이트 데이터'를 추출하여 캐싱 함수에 전달
    file_bytes = uploaded_file.getvalue()
    raw_img_resized, processed_img = preprocess_image(file_bytes)
    
    with st.expander("원본 및 전처리된 이미지 확인하기"):
        col1, col2 = st.columns(2)
        with col1:
            st.image(raw_img_resized, caption="원본 (용량 최적화됨)", use_container_width=True)
        with col2:
    ...
    if st.button("선생님께 새 문제 질문하기"):
        ...
        with st.spinner("이미지를 분석하고 풀이를 고민 중입니다."):
            ...
            try:
                ...
                st.error("앗! 선생님이 한 번에 너무 많은 질문을 처리하느라 지쳤어요. (API 1분 할당량 초과) 딱 1분만 기다렸다가 다시 질문해 주세요!")
